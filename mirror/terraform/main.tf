...
resource "oci_core_instance" "mirror" {
  compartment_id      = var.compartment_ocid
  ...
  create_vnic_details {
    subnet_id        = oci_core_subnet.mirror.id
...
data "oci_core_vnic_attachments" "mirror" {
  compartment_id = var.compartment_ocid
  instance_id    = oci_core_instance.mirror.id
}

data "oci_core_vnic" "mirror" {
  vnic_id = data.oci_core_vnic_attachments.mirror.vnic_attachments[0].vnic_id
}
